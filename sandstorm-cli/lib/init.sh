...
# Ticket provider selection is now done in Sandstorm Desktop (Project Settings → Ticketing).
# No scripts are copied — ticket operations are built into the desktop app.
...
# ---------------------------------------------------------------------------
# Install ticket skills (provider-neutral)
# ---------------------------------------------------------------------------
# Ticket provider scripts are no longer copied per-project — operations are
# built into Sandstorm Desktop. Only the sandstorm skill files are installed.
TICKET_SKILLS_DEST="$SANDSTORM_CONFIG_DIR/skills"
GITHUB_SKILLS_SRC="$SANDSTORM_DIR/templates/github/skills"
if [ -d "$GITHUB_SKILLS_SRC" ]; then
  if [ ! -d "$TICKET_SKILLS_DEST" ] || [ -z "$(ls -A "$TICKET_SKILLS_DEST" 2>/dev/null)" ]; then
    mkdir -p "$TICKET_SKILLS_DEST"
    cp "$GITHUB_SKILLS_SRC"/*.md "$TICKET_SKILLS_DEST/" 2>/dev/null || true
    echo "  Installed ticket skills to .sandstorm/skills/"
  else
