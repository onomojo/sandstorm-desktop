...
describe('AgentSession', () => {
  ...
  it('renders "Scheduled" badge on user messages marked as scheduled', async () => {
    // Pre-populate store with a scheduled message
    useAppStore.getState().updateAgentSession('test-tab', {
      messages: [
        { role: 'user', content: 'Scan open issues', scheduled: true },
    ...
    expect(screen.getByText('Scan open issues')).toBeDefined();
  });

  it('does not render "Scheduled" badge on normal user messages', async () => {
    useAppStore.getState().updateAgentSession('test-tab', {
      messages: [
    ...
    expect(screen.queryByTestId('scheduled-badge')).toBeNull();
    expect(screen.getByText('Hello agent')).toBeDefined();
  });

  it('marks last user message as scheduled when schedule:dispatched event fires', async () => {
    await act(async () => {
      render(<AgentSession tabId="test-tab" projectDir="/test" />);
    });

    // Add a user message via the agent:user-message event
    const userMsgHandler = eventHandlers['agent:user-message:test-tab'];
    act(() => {
      userMsgHandler('Scheduled prompt text');
    });

    // Fire the schedule:dispatched event
    const scheduledHandler = eventHandlers['schedule:dispatched'];
    expect(scheduledHandler).toBeDefined();
    act(() => {
      scheduledHandler({ tabId: 'test-tab', scheduleLabel: 'Triage' });
    });

    // The badge should now appear
    const badge = screen.getByTestId('scheduled-badge');
    expect(badge).toBeDefined();
  });

  it('preserves scheduled badge on user message after agent:done history refetch', async () => {
    await act(async () => {
      render(<AgentSession tabId="test-tab" projectDir="/test" />);
    });

    // Fire a user message and mark it as scheduled
    const userMsgHandler = eventHandlers['agent:user-message:test-tab'];
    act(() => {
      userMsgHandler('Scheduled prompt text');
    });
    ...
    // Badge should be visible before done
    expect(screen.getByTestId('scheduled-badge')).toBeDefined();

    // Simulate agent:done — history includes the scheduled user message and assistant reply
    api.agent.history.mockResolvedValue({
      messages: [
        { role: 'user', content: 'Scheduled prompt text' },
    ...
    // Badge should still be present after history sync
    expect(screen.getByTestId('scheduled-badge')).toBeDefined();
  });

  it('handles schedule:dispatched event gracefully when tab has no messages', async () => {
    await act(async () => {
      render(<AgentSession tabId="test-tab" projectDir="/test" />);
    });

    // No messages have been added to the session yet
    const state = useAppStore.getState().agentSessions['test-tab'];
    expect(!state || state.messages.length === 0).toBe(true);

    // Fire the schedule:dispatched event — should not throw
    const scheduledHandler = eventHandlers['schedule:dispatched'];
    expect(scheduledHandler).toBeDefined();
    expect(() => {
      act(() => {
    ...
    // No badge should appear since there are no messages to mark
    expect(screen.queryByTestId('scheduled-badge')).toBeNull();
  });
